...
def EdgeDistanceDist(abc): ##Working In current form
    ...
    plt.bar([i for i in range(0,NLangFeatures+1)],15*DistanceFreq)
    #plt.hist(DistanceFreq)
    plt.title(f"Distance Between Neighbouring Nodes for the Peference model at T={abc} ")
    plt.xticks(ticks=[i for i in range(0,NLangFeatures+1)],labels=[str(i/8) for i in range(0,NLangFeatures+1)])
    
    plt.xlabel(f"Distance $h(i,j)$")
    plt.ylabel("Frequency")
    MeanDist = 0
    for i in range(0,9):
        MeanDist+= DistanceFreq[i]*(i/8)
    #plt.figtext(0.5, -0.3, subtitle_string , wrap=True, horizontalalignment='center', fontsize=8)
    #plt.yscale("log")#, kwargs)
    print(f"Mean={MeanDist/np.sum(DistanceFreq)}")
    return DistanceFreq
...
#EnergyMatrix = np.zeros([len(TempRange),16*256])+np.nan
import numpy as np 

IndicesICareFor = (23,23)
ThreshBigArray = np.zeros((4,4000))
for x in range(0,len(IndicesICareFor)):
    i = IndicesICareFor[x]
    WholeVector = np.zeros(1)
    Data = ResultsFor('Pref', float(TempRange[i]), 300)
    for j in range(0,len(Data.files)):
        Population = 0
        Population = LatticeGenerate(Data[str(j)])
        Result = LanguageVector(Data[str(j)])
        WholeVector = np.concatenate((WholeVector,Result))
        ThreshBigArray[x,:] = np.concatenate((WholeVector,np.zeros(4000-len(WholeVector))))
        #OtherArray += Result
    #     plt.plot(Result,label=f"Attempt{j}")
        SpinVisualiser()
        DFreq = EdgeDistanceDist(ThreshTempLabels[i])
        print(f"percentage {min(DFreq)/(np.sum(DFreq))},min {min(DFreq)}")
        #E = FastEnergy()
        #print(E)
        #EnergyMatrix[i,j] = E
        #print(f"{i*10+j}% done")
...
#%%
for x in range(0,len(IndicesICareFor)):
     y = np.log10(((ThreshBigArray[x,:]))[np.where(ThreshBigArray[x,:]>0)])
     plt.hist((y-y.min())/y.max()-y.min())
     plt.figure()
#np.save("Thesh7.17.27.37.npy", ThreshBigArray)

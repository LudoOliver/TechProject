...
#%%      
"""Mean Energy Plotting"""  
plt.cla()
EnergyVector= np.nanmean(EnergyMatrix,axis=1) 
for i in range(15):
    plt.plot(ThreshTempValues , EnergyMatrix[:,i],alpha=0.1)
...
""" VariancePlotting """
plt.cla()
EnergyVar= np.nanvar(EnergyMatrix,axis=1) 
#for i in range(15):
    #lt.plot(ThreshTempValues , EnergyMatrix[:,i],alpha=0.1)
#     stats.probplot(ThreshBigArray[i,:], dist="norm", plot=pylab)
plt.plot(ThreshTempValues ,EnergyVar,color='b')   
plt.xlabel("Temperature")
...
plt.vlines(0.57, -2.5, 3,colors='r',linestyles='--')#, kwargs)
plt.title("Critical Point of Threshold Model")#, kwargs)
plt.xlabel("Temperature")
...
for j in range(0,len(TempRange)):
    ...
    if ThreshTempLabels[j]=='0.57':
        plt.plot(x,density(x),label=f"T={TempRange[i]}",alpha=0.6,linewidth=1,color='b')
    plt.plot(x,density(x),label=f"T={TempRange[i]}",alpha=0.2,linewidth=0.5)
